...
void AutomationEditor::paint(Graphics& g)
{
	...
	for(int i = 0; i < 4; i++)
	{
		...
		else
		{
			...
			int low = inf.m_minRange * (LANE_WIDTH - (TAB_RADIUS * 2)) + TAB_SPACING + TAB_RADIUS,
				high = inf.m_maxRange * (LANE_WIDTH - (TAB_RADIUS * 2)) + TAB_SPACING + TAB_RADIUS;
			if(high < low)
			{
...
void AtomKnob::paint(Graphics & g)
{
	...
	double x = (std::sin(angle) * w / 2) + (w / 2),
	       y = (std::cos(angle) * h / 2) + (h / 2);
	g.setColour(MID_LAYER);

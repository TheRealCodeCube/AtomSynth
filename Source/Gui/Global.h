...
{
	constexpr int
		CORNER_SIZE = 4,      //Used for rounded rectangles.
...
#define CB(X, Y, W, H) CD(X), CD(Y), CS(W), CS(H)                       //Cell bounding box (Use in setBounds function of a component.)
	...
	constexpr int
		CELL_SIZE = 30,          //Used for generic layouts.
...
#define HB(X, Y, W, H) HD(X), HD(Y), HS(W), HS(H)                            //Half cell bounding box. Same as CB, except with half cells.
	...
	constexpr int
		...
		VST_WIDTH = CD(30),
		GUI_HEIGHT = CD(20),
		VST_HEIGHT = GUI_HEIGHT + HEADER_HEIGHT,
		GUI_WIDTH = CD(12),
		BUTTON_TRAY_WIDTH = GUI_WIDTH - HEADER_WIDTH - SPACING;

	constexpr double
		MIN_TEXT_SCALE = 0.8, //Used for drawing fitted text.

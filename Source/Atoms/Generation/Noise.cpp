...
void NoiseController::automatedControlChanged(AutomatedControl * control, bool byUser)
{
	  ...
	  m_parent(parent)
{
	...
	m_previousValues.resize(2, std::vector<double>());
...
void NoiseAtom::execute()
{
	...
	for(int c = 0; c < AudioBuffer::getDefaultChannels(); c++)
	{
		for(int s = 0; s < AudioBuffer::getDefaultSize(); s++)
		{
			...
			{
				m_outputs[0].setValue(c, s, (m_previousValues[c][0] * m_lengthTimer[c] + m_previousValues[c][1] * (1 - m_lengthTimer[c])) * * amplitudeIter);
			}
			else if(type == 2) //smooth
			{
				double interpolation = 6 * pow(m_lengthTimer[c], 5) - 15 * pow(m_lengthTimer[c], 4) + 10 * pow(m_lengthTimer[c], 3);
				m_outputs[0].setValue(c, s, (m_previousValues[c][0] * interpolation + m_previousValues[c][1] * (1 - interpolation)) * * amplitudeIter);
			}
